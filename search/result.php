...
<body>
	...
	/*	$sql="select A_NAME,LOCATION,CITY,PIN_CODE,ROOM_TYPE,TENANT_PREF,RENT,FOOD_SERIVES,FURNISHED,mobno FROM accomodation where CITY='$s1' AND ROOM_TYPE='$s2' AND TENANT_PREF='$s3'  AND rent<='$s4'";*/
		 $result=mysql_query("select A_NAME,LOCATION,CITY,PIN_CODE,ROOM_TYPE,TENANT_PREF,RENT,FOOD_SERIVES,FURNISHED,mobno from accomodation where (CITY='$s1' AND                          ROOM_TYPE='$s2' AND TENANT_PREF='$s3' AND RENT<='$s4')");
		 if(!$result)
		 {
			  die("Database Query Failed " .mysql_error()); 
		 }
	 ...
	 <tr>   <th>Contact No. :</th> <td> ".$row['mobno']."</td></tr>
        </table>
